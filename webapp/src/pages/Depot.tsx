...
export default function Depot() {
  ...
  const grouped = entries.reduce(
    (acc, e) => {
      const cat =
        Object.entries(CATEGORIES).find(([, repos]) => repos.includes(e.repo))?.[0] || "Other";
      if (!acc[cat]) acc[cat] = [];
      acc[cat].push(e);
      return acc;
    },
  ...
  const filteredEntries = repoFilter ? entries.filter((e) => e.repo === repoFilter) : entries;

  const filteredGrouped = repoFilter
    ? (() => {
        const cat =
          Object.entries(CATEGORIES).find(([, repos]) => repos.includes(repoFilter))?.[0] ||
          "Other";
  ...
  return (
    <div data-testid="depot-page" className="p-6 max-w-6xl mx-auto">
      ...
      {regenResult && (
        <div className="bg-zinc-800 rounded-md p-3 mb-4 text-sm text-zinc-300 border border-zinc-700">
          {regenResult}
        </div>
      )}
                ...
                >
                  {playing === v.video_path ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      ...
                      <div className="relative z-10 h-12 w-12 rounded-full bg-black/60 flex items-center justify-center group-hover:bg-amber-600/80 transition-colors">
                ...
                <div className="p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        <RotateCcw
                          className={`h-3.5 w-3.5 ${regenBusy === v.repo ? "animate-spin" : ""}`}
                        />
                      </button>
                      <button
                  ...
                  {expanded === v.repo && v.script && (
                    <pre className="mt-2 bg-zinc-950 rounded p-2 text-xs text-zinc-400 overflow-auto max-h-40">
                      {v.script}
                    </pre>
                  )}
                </div>
      ...
      {entries.length === 0 && (
        <div className="text-center text-zinc-600 py-12">
          No videos in depot. Generate one first.
        </div>
      )}
    </div>
  );
