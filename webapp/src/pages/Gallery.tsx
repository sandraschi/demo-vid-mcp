...
interface VideoItem { name: string; repo: string; size_kb: number; created: string; }
...
export default function Gallery() {
  ...
  return (
    <div data-testid="gallery-page" className="p-6 max-w-5xl mx-auto">
      ...
      {!loading && videos.length === 0 && <div className="text-center text-zinc-600 py-12">No videos yet. Generate one from the Generate page.</div>}

      <div className="grid grid-cols-2 gap-4">
        {videos.map((v) => (
          <div key={v.name} className="bg-zinc-900 rounded-lg border border-zinc-800 overflow-hidden hover:border-zinc-700 transition-colors">
            ...
            <div className="p-3">
              ...
              <div className="text-xs text-zinc-500 mt-1">{v.repo} · {v.size_kb} KB</div>
            </div>
          </div>
        ))}
      </div>
