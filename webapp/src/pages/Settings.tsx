...
import { Settings as SettingsIcon, RefreshCw, Wifi, WifiOff, Loader } from "lucide-react";

interface Provider { name: string; port: number; detected: boolean; models?: string[]; }
  ...
  const probe = useCallback(async () => {
    ...
    try {
      const [h, d] = await Promise.all([
        ...
        fetch("/api/llm/discover").then(r => r.json()).catch(() => ({providers: []})),
      ]);
    ...
    <div data-testid="settings-page" className="p-6 max-w-3xl mx-auto">
      ...
      <div className="bg-zinc-900 rounded-lg p-4 border border-zinc-800 mb-4">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <button onClick={probe} disabled={probing} className="ml-auto p-1.5 rounded text-zinc-500 hover:text-white hover:bg-zinc-800 cursor-pointer">
      ...
      <div className="bg-zinc-900 rounded-lg p-4 border border-zinc-800 mb-4">
        ...
        <div className="space-y-2 mb-4">
          {providers.length === 0 && probing && (
            <div className="flex items-center gap-2 text-sm text-zinc-500"><Loader className="h-4 w-4 animate-spin" /> Probing providers...</div>
          )}
          {providers.length === 0 && !probing && (
            <div className="text-sm text-amber-500">No local LLM detected. Install Ollama or LM Studio to enable AI features.</div>
          )}
          {providers.map(p => (
            <div key={p.name} className="flex items-center gap-3 p-2 rounded-md bg-zinc-950 border border-zinc-800">
              {p.detected ? <Wifi className="h-4 w-4 text-green-500" /> : <WifiOff className="h-4 w-4 text-zinc-600" />}
              <span className="text-sm text-zinc-300 flex-1">{p.name}</span>
              <span className="text-xs text-zinc-600">:{p.port}</span>
              <span className={`text-xs ${p.detected ? "text-green-500" : "text-zinc-600"}`}>{p.detected ? "Detected" : "Not found"}</span>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            >
              {providers.filter(p => p.detected).length === 0 && <option value="">No provider detected</option>}
              {providers.filter(p => p.detected).map(p => <option key={p.name} value={p.name}>{p.name}</option>)}
            </select>
          </div>
          <div>
            ...
            >
              {(!currentProvider?.models || currentProvider.models.length === 0) && <option value="">No models</option>}
      ...
      <div className="bg-zinc-900 rounded-lg p-4 border border-zinc-800">
        ...
        <div className="text-sm text-zinc-500 space-y-1">
          ...
          <div className="text-xs text-zinc-600 mt-2">Fleet repo: {providers.length} providers probed · {providers.filter(p => p.detected).length} detected</div>
        </div>
      </div>
    </div>
  );
