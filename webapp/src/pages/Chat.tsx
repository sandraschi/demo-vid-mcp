...
import { Send, Eraser, Download, Sparkles, User, Bot } from "lucide-react";
...
const PERSONALITIES: Record<string, string> = {
  "Research Assistant": "You are a thorough research assistant. Provide detailed, well-structured answers with citations where relevant.",
  "Expert Reviewer": "You are a code and architecture reviewer. Analyze critically, point out issues, suggest improvements.",
  "Quick Summarizer": "You summarize concisely in 3-5 bullet points. No fluff, no preamble.",
  "Custom": "",
};

const EXAMPLE_PROMPTS = [
  { group: "Discovery", prompts: ["How do I generate a video?", "List all available tools", "What ports does this service use?"] },
  { group: "Scripting", prompts: ["Draft a narration script for chitchat", "Validate this YAML script", "How do I add a desktop capture step?"] },
  { group: "Troubleshooting", prompts: ["Why is my video blank?", "No voiceover in output", "Playwright capture hangs"] },
];
...
export default function ChatPage() {
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      const personalityPrompt = personality === "Custom" ? customPrompt : PERSONALITIES[personality] || "";
      ...
      const mcpMessages = [{ role: "system", content: systemContent }, ...messages, userMsg].map(m => ({ role: m.role, content: m.content }));
      ...
      if (data.success && data.message?.content) {
        setMessages(prev => [...prev, { role: "assistant", content: data.message.content, ts: new Date().toISOString() }]);
      } else {
        setMessages(prev => [...prev, { role: "assistant", content: data.error || "No response from provider", ts: new Date().toISOString() }]);
      }
    } catch (e) {
      setMessages(prev => [...prev, { role: "assistant", content: `Error: ${e}`, ts: new Date().toISOString() }]);
    } finally {
      setBusy(false);
    }
  ...
  const handleExport = useCallback(() => {
    ...
    const text = messages.map(m => `[${new Date(m.ts || "").toLocaleString()}] ${m.role === "user" ? "You" : "AI"}: ${m.content}`).join("\n\n");
    const blob = new Blob([text], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `demo-vid-mcp-chat-${Date.now()}.txt`; a.click();
  ...
  return (
    <div data-testid="chat-page" className="flex flex-col h-full">
      {/* Controls Bar */}
      <div data-testid="chat-controls" className="flex items-center gap-3 px-6 py-3 border-b border-zinc-800 bg-zinc-950 shrink-0 flex-wrap">
        <select
          value={personality}
        ...
        {personality === "Custom" && (
          <input value={customPrompt} onChange={e => setCustomPrompt(e.target.value)}
            className="bg-zinc-800 text-zinc-200 border border-zinc-700 rounded-md px-2 py-1.5 text-xs w-48" placeholder="Custom system prompt..." />
        )}

        <div className="flex items-center gap-1.5 ml-auto">
          <div className={`w-2 h-2 rounded-full ${providerOk === null ? "bg-zinc-500" : providerOk ? "bg-green-500" : "bg-red-500"}`} />
          <span className="text-xs text-zinc-500">{provider} {model ? `· ${model}` : ""}</span>
        </div>

        <button onClick={handleExport} disabled={messages.length === 0}
          data-testid="chat-export"
          className="p-1.5 rounded text-zinc-500 hover:text-white hover:bg-zinc-800 disabled:opacity-30 cursor-pointer" title="Export">
          <Download className="h-4 w-4" />
        </button>
        <button onClick={handleClear} disabled={messages.length === 0}
          data-testid="chat-clear"
          className="p-1.5 rounded text-zinc-500 hover:text-red-400 hover:bg-zinc-800 disabled:opacity-30 cursor-pointer" title="Clear">
      ...
      <div data-testid="chat-messages" className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        {messages.length === 0 && (
          <>
            <div className="text-center py-12">
              ...
              <p className="text-sm text-zinc-600">Generate, script, or troubleshoot your fleet video pipeline.</p>
            </div>

            <div data-testid="example-prompts" className="space-y-3">
              {EXAMPLE_PROMPTS.map(group => (
                <div key={group.group}>
                  <div className="text-xs text-zinc-600 mb-1.5 font-medium uppercase tracking-wide">{group.group}</div>
                  <div className="flex flex-wrap gap-2">
                    {group.prompts.map(p => (
                      <button key={p} onClick={() => handleExampleClick(p)}
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex gap-3 ${m.role === "user" ? "justify-end" : ""}`}>
            {m.role === "assistant" && <div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center shrink-0"><Bot className="h-4 w-4 text-amber-400" /></div>}
            <div className={`max-w-[75%] ${m.role === "user" ? "order-1" : ""}`}>
              ...
              <div className={`rounded-lg px-4 py-2.5 text-sm leading-relaxed whitespace-pre-wrap ${
                m.role === "user" ? "bg-amber-600/20 text-zinc-200 border border-amber-700/30" : "bg-zinc-900 text-zinc-300 border border-zinc-800"
              }`}>
                {m.content}
              </div>
            </div>
        ...
        {busy && (
          <div className="flex gap-3">
            <div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center shrink-0"><Bot className="h-4 w-4 text-amber-400" /></div>
      ...
      <div className="border-t border-zinc-800 p-4 bg-zinc-950 shrink-0">
        <div className="flex gap-3 max-w-3xl mx-auto">
          <input
            ...
            placeholder={providerOk === false ? "No LLM detected — start Ollama or LM Studio" : "Ask about demo videos..."}
